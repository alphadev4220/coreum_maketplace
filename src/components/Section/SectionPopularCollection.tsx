...
export interface SectionPopularCollectionProps {
  className?: string;
  itemClassName?: string;
}

const SectionPopularCollection: FC<SectionPopularCollectionProps> = ({
  className = "",
}) => {
  ...
  const getHotCollections = (time: any, limit: any) => {
    axios
      ...
      .then((result) => {
        const colData = result.data.data.filter((item) => item.collection_info?.bannerURL !== "" && item.collection_info?.bannerURL !== undefined)
...
export default SectionPopularCollection;
