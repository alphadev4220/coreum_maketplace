...
function SidebarChat({ chatroomtile, currentChat, currentUser, arrivalChat }) {
    ...
    const getLatestMessage = async () => {
      try {
        const response = await axios.get(
          API_URL + "api/messages/latest/" + chatroomtile?._id
        );
  ...
  const getUnreadCount = async () => {
    try {
      ...
      if (resp.data > 0) {
        document.title =
          document.title.replace(/\(\d+\)/g, "") + ` (${resp.data})`;
      } else {
  ...
  return (
    <div
      className={isSelected ? "sidebarchat sidebarchat-select" : "sidebarchat"}
    >
      <div className="flex items-center">
        <img
          className="amigo-profilepic"
          src={
            user?.avatar ? `${config.UPLOAD_URL}uploads/${user.avatar}` : defaultAvatar
          }
          alt=""
        />
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-3 justify-end">
          <span className="latest_time">
            {latestAt ? latestAt.replace(" ago", "") : "just now"}
          </span>
        </div>
        <div className="flex gap-3 justify-end">
          ...
          >
            {favorite ? (
              <FaStar size={20} color="#12A4FF"></FaStar>
            ) : (
              <FiStar size={20} color="#8f9199"></FiStar>
            )}
          </button>
        </div>
      </div>
