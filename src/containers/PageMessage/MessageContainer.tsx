    ...
    const getMessages = async () => {
      try {
        ...
        for (let i = 0; i < response.data.length; i++) {
          if (response.data[i].senderId !== user?._id) {
            setLastSeen(response.data[i]?.updatedAt);
            break;
          }
        }
        ...
        <>
          <div className="chatroom-header">
            <div className="chatroom-chatinfo">
              <img
                ...
                src={
                  amigo?.avatar
                    ? `${config.UPLOAD_URL}uploads/${amigo.avatar}`
                    : defaultAvatar
                }
                alt=""
              />
              <div className="chatroom-chatinfo-right">
                ...
                <div className="chatroom-top-header">
                  <span>
                    Last seen: {lastSeen !== "" ? format(lastSeen) : "never"}
                  </span>
                  <span> | </span>
                  <span>
                    Local time:{" "}
                    {lastSeen !== ""
                      ? moment(lastSeen).format("MMM DD, YYYY, hh:mm A")
                      : ""}
                  </span>
                </div>
              </div>
          ...
          <div
            className={`chatroom-container ${
              isMobile ? "h-[calc(100%-199px)]" : "h-[calc(100%-99px)]"
            }`}
          >
